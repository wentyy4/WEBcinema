    ...
    <>
      ...
      <section className="container" style={{ marginTop: 56 }}>
        ...
        <div className="grid-4" style={{ marginTop: 24 }}>
          {visible.map(f => (
            <div key={f.id} className="card movie-card food-card" style={{ padding: 14, display:'flex', flexDirection:'column', minHeight: 350 }}>
              <FoodIcon hue={f.hue}/>
              <div style={{ marginTop: 14, minHeight: 40, fontSize: 15, lineHeight: 1.2, fontWeight: 600 }}>{f.name}</div>
              <div style={{ minHeight: 42, fontSize: 12.5, lineHeight: 1.35, color:'var(--text-mute)' }}>{f.desc}</div>
      ...
      <style>{`
        ...
        @media (max-width: 560px) { .food-card { min-height: 0 !important; } }
      `}</style>
    </>
  );
}
